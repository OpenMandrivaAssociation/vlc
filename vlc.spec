...
# vlc-cache-gen segfaults while scanning the just-built plugins. The
# cache is regenerated by the file trigger; do not fail the build on it.
sed -i 's#./vlc-cache-gen$(EXEEXT) `realpath ../modules`#./vlc-cache-gen$(EXEEXT) `realpath ../modules` || true#' bin/Makefile
sed -i 's#"$(DESTDIR)$(vlclibdir)/vlc-cache-gen$(EXEEXT)"#"$(DESTDIR)$(vlclibdir)/vlc-cache-gen$(EXEEXT)" || true#' Makefile
